    ...
    <div className={cn('flex gap-4 animate-fade-in', isUser ? 'flex-row-reverse' : 'flex-row')}>
      ...
      <div
        className={cn(
          ...
          isUser
            ? 'bg-secondary'
            : 'neon-border box-glow-sm'
        )}
        style={!isUser ? { background: 'linear-gradient(135deg, hsl(263 70% 58% / 0.2), hsl(187 85% 53% / 0.2))' } : undefined}
      >
        {isUser ? (
              ...
              <div className="prose prose-sm prose-invert max-w-none [&>*:first-child]:mt-0 [&>*:last-child]:mb-0">
                <ReactMarkdown
                  components={{
                    ...
                    a: (props) => <a target="_blank" rel="noopener noreferrer" className="text-accent hover:underline" {...props} />,
                    blockquote: (props) => <blockquote className="border-l-2 border-accent/50 pl-3 italic text-muted-foreground text-sm" {...props} />,
                    img: ({ src, alt }) => (
                      <div className="relative group my-2">
                        ...
                        <a
                          ...
                          className="absolute top-2 right-2 bg-background/80 backdrop-blur-sm p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity box-glow-sm"
                        >
